    ...
    async function fetchAverages() {
      ...
      for (const category of categories) {
        const res = await fetch(
          `/api/ratings?rateeId=${rateeId}&category=${encodeURIComponent(category)}`
        );
        const data = await res.json();
        if (data.length > 0) {
          avgs[category] = (data.reduce((sum, r) => sum + (r.stars || 0), 0) / data.length).toFixed(
            2
          );
        } else {
          avgs[category] = 'N/A';
            ...
            <div>
              {[1, 2, 3, 4, 5].map(star => (
                <span
                  ...
                  style={{
                    cursor: 'pointer',
                    color: ratings[category] >= star ? '#fbbf24' : '#d1d5db',
                    fontSize: '2rem',
                  }}
