...
export default function RatingSubmissionForm({
  ...
  if (!ratedUserId || !raterUserId) {
    return (
      <div className="text-center text-red-600 font-semibold my-8">
        User not found or invalid rating request.
      </div>
    );
  }
  ...
  const handleStarClick = (key: string, value: number) => {
    setCriteria(prev => {
      ...
      updated.overall = Math.round(
        (updated.professionalism + updated.timeliness + updated.communication) / 3
      );
      return updated;
    });
  };
  ...
  return (
    <form
      ...
      className="p-4 border rounded-lg bg-white max-w-xl mx-auto my-6 space-y-4"
    >
          ...
          <div>
            {[1, 2, 3, 4, 5].map(star => (
              <span
                ...
                style={{
                  cursor: 'pointer',
                  color: criteria[key] >= star ? '#fbbf24' : '#d1d5db',
                  fontSize: '2rem',
                }}
      ...
      <button
        ...
        disabled={
          submitting || !criteria.professionalism || !criteria.timeliness || !criteria.communication
        }
      >
